...
void DFS(MGraph& g, int src, bool visited[], void (*visit)(VertexInfo)) {
    if (src >= g.n) {
        return;  // 超出范围
    }
    ...
    for (int i = 0; i < g.n; ++i) {
        if (!visited[i] && g.edges[src][i] != INF) {
            DFS(g, i, visited, visit);  // 遍历未访问的邻接点
        }
    }
}

// 深度优先遍历图g，visit为访问函数
void DFSTraverse(MGraph& g, void (*visit)(VertexInfo)) {
    bool visited[MAXSIZE] = {false};  // 初始化访问标记数组
    for (int i = 0; i < g.n; ++i) {
        if (!visited[i]) {
            DFS(g, i, visited, visit);  // 若未访问，则递归访问该顶点
        }
    }
}
